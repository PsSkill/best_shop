...
exports.get_dashboard_data = async (req, res) => {
  try {
    ...
    ROUND(IFNULL(AVG(total_price / NULLIF(quantity, 0)), 0)) AS rate_of_product
    FROM
    (
    ...
    stock ON
        CASE
            ...
            WHEN intervals.time_interval = 'Between 30 to 180 days' THEN DATEDIFF(CURRENT_DATE(), date) >= 30 AND DATEDIFF(CURRENT_DATE(), date) <= 180
            WHEN intervals.time_interval = 'Between 180 to 365 days' THEN DATEDIFF(CURRENT_DATE(), date) > 180 AND DATEDIFF(CURRENT_DATE(), date) <= 365
        END
    GROUP BY
    intervals.time_interval;`;
